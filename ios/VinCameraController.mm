...
@property (nonatomic, assign) BOOL isAlbumRecognizing;
...
- (void)initRecognizeCore {
    ...
    if (nRet != 0) {
        ...
        //成功
        NSInteger day = [VinCameraController calculateTheRemainingDaysOfLicenceWithDeadLine:self.vinTyper.nsEndTime];
...
- (void)deliverResultAndImage:(UIImage *)srcImage {
    ...
    [dict setValue:self.vinTyper.nsResult forKey:@"vinStr"];
    ...
    NSData * vinData = UIImageJPEGRepresentation(self.vinTyper.resultImg, 0.8);

    [vinData writeToFile:vinPath atomically:YES];
    [dict setValue:vinPath forKey:@"vinImagePath"];
    
    NSString * areaPath = [tmpDir stringByAppendingPathComponent:@"areaVinImagePath.jpg"];

    NSData * areaData = UIImageJPEGRepresentation(vinImage, 0.8);
    [areaData writeToFile:areaPath atomically:YES];
    [dict setValue:areaPath forKey:@"areaVinImagePath"];
    if (self.getVinResultFromOCR) {
      self.getVinResultFromOCR(dict.copy);
    }
